...
\end{lemma}
\begin{proof}
With the simplifications from before, we have
% \be \label{nulivp}
...
Thus $\nu$ has a unique zero on $(0, \za)$.  Moreover, $\nu(0) = \beta_0 \a > 0$
and $\underset{r\to 0}{\lim} r\nu'(r) = 0$. 
%Thus, $\nu(0) > 0$.  
{\red Maybe no display to fit on one page.} Furthermore, if $\a\in N$, then \[
\nu(\za) = \za u'(\za) < 0 \] by Lemma \ref{genlem2} and so $\nu$ has a unique
zero $\rho_0$ in $[0,\za]$.
