...
\eqref{genuivp} below. The method is due to Coffman \cite{coffm} and was later
generalised by Kwong \cite{kwong} for the autonomous problem with arbitrary $n$.
...
following hypotheses are made: $V(r)$ is continuously differentiable
\be \label{vh1} 
V \in C^1 \left( (0, \infty) \right); 
\ee
...
% TODO: P, G and N definitions

\subsection{Definitions of solution sets}
In this chapter, the solution sets $P$, $G$ and $N$ are defined as
\be \label{nset}
...
\ee
\be \label{gset} 
G\coloneqq\left\{\;\a>0 \;\middle|\; u(r,\a)>0 \;\text{for all}\;r>0\;
    \text{and}\;
...
% u'(r_0,\a)=0 \;\text{and}\; u(r,\a)>0 \;\text{for}\; r\in(0,r_0] \;\Big\}.
u(r,\a)>0 \;\text{for all}\; r>0\;\text{and}\;\a\notin G \;\Big\}
\ee
...
\begin{theorem}
    ...
    where $f, g$ and $G$ are continuous. Let $(\mu, \nu)$ be a subinterval in
...
\be \label{upos}
\begin{dcases}
    u(r) > 0 \quad \text{for } r\in[0, \infty) \quad \text{if} ~ \a \in P \cup G, \\
    u(r) \geq 0 \quad \text{for } r\in[0,\za] \quad \text{if} ~ \a \in N.
\end{dcases}
\ee
...
In particular, we have $E(0) \geq 0 ~ \text{for} ~ \a \in G \cup N$. 
% This has the following consequence. 
Then the left hand side is non-negative in the inequality {\red(why?)}
\be \label{lyasolv}
...
This implies by \eqref{lyadef} 
%Lemma \ref{genlem1} 
that $E(r_0) <
0$, which contradicts \eqref{epos}. Therefore, $u'(r) < 0$ for all $r
\in (0, \za)$. 
%{\red short?} 
...
We use $u(\za) = 0$ and $r = 0$, such that \eqref{partint} reads
\be\label{partintshort}
    w(\za) \za u'(\za).
\ee
...
then $w(\za) \geq 0$. Hence, the left hand side of \eqref{genlem3neq} is
non-positive. However, the right hand side is positive. By this contradiction,
$w$ has at least one zero in $(0, \za)$. 
...
such that \eqref{uwdiff} integrated from $0$ to $r$ reads
\be \label{uwli}
...
We use $u(0) = \a > 0$ and $w(r) = 1$ from \eqref{genuivp}
and \eqref{genwivp} to conclude that
\be \label{uwpos}
    \frac{u(r)}{w(r)} > 0 \quad \text{on}~ (0, \infty).
\ee

We refer to \cite[Proof of Lemma  2, p. 487]{gen11} for the remaining details of
this proof. The proof requires lemmata from the earlier work \cite{genthes}. In
...
\]
This contradicts \eqref{uwdev}. Hence, $w$ has at least one zero on $(0,
\infty)$.
% 
...
This section is dedicated to introducing functions related to the initial value
problems \eqref{genuivp} and \eqref{genwivp}. We introduce the functions $\tr, ~
\rb, ~ \nub(r) ~ \text{and} ~ \phib(r).$ Furthermore, in \Cref{genlem4}, we will
define the function $\sib$ as the inverse of $\Xi(r)$, which is a transformation
of $\xi(r)$ that relates the zero of $\phib(r)$ to $\beta$. In \Cref{genlem5},
we conclude that the zero of $\nub(r)$ and $\phib(r)$ coincide in $\beta_0$.
From then on, we will fix $\beta = \beta_0$. By this construction
$\nu(r)=\nu_{\beta_0}(r)$ has a unique zero. This is used in \Cref{genlem6} to
show that $w(r)$ has a unique zero.
...
Lastly, let $\a\in N$. A similar argument holds for $\a\in G$. Then $\za$ is the
first zero of the solution $u(r,\a)$. Also, by \Cref{genlem2} the derivative
$u'(r,\a)$ is negative everywhere.  Hence, by the definition of $\theta(r)$ in
...
We claim that $\nub''(r)$ satisfies the differential equation
\be \label{gennuivp}
    \nub''(r) + \frac{1}{r} \nub'(r) - \lambda \nub(r) + p V(r) u(r)^{p-1}
    \nub(r) = \phib(r),
\ee
...
We calculate the terms of the left-hand side of \eqref{gennuivp} as
\begin{gather*}
...
We add all the terms and write the differential equation as
\begin{multline}\label{nudelong}
    \nub''(r) + \frac{1}{r} \nub'(r) - \lambda \nub(r) + p V(r) u(r)^{p-1}
    \nub(r) \\
...
and use these substitutions to simplify \eqref{nudelong} to
% \begin{multline}
%     \nub''(r) + \frac{1}{r} \nub'(r) - \lambda \nub(r) + p V(r) u(r)^{p-1}
%     \nub(r) \\
...
\begin{multline}\label{nudeshort}
\nub''(r) + \frac{1}{r} \nub'(r) - \lambda \nub(r) + p V(r) u(r)^{p-1}
\nub(r) \\
...
\begin{proof}
We fix $\beta>0$. We write \eqref{phibdef} as
\be\label{phibdefshort} 
\phib(r) = V(r) u(r)^p \left[ \beta (p-1) - 2 - \xi(r) \right], 
\ee
% w
% To simplify $\phib(r)$ as defined in \eqref{phibdef}, we
% define the function
% Then $\phib(r)$ reads
where
\be \label{xidef}
...
The sign of \eqref{phibdefshort}
%$\phib(r)$ 
depends on the term in brackets, since $V > 0$ by
\eqref{vh2} and $u(r)^p > 0$ by \eqref{upos}. We are interested in the zero of
$\phib$. We define the function $\Xi(r)$ such that 
\[ 
...
Thus, $h_0 \geq -k > -2$ by \cite[Remark 2.1]{gen11}. On the other hand, if
$V(0) < \infty$, then
\be \label{xivfin}
\xi(0) = - \frac{2 \lambda}{\a^{p-1} V(0)}.
\ee

We solve \eqref{xivfin} for $\a$ 
\[ 
    \a > \left[\frac{\lambda}{V(0)}\right]^\frac{1}{p-1},
\]
to conclude that $\xi(0) > -2$ for $\a\in G\cup N$ by \Cref{genlem1}.
%which is true by \Cref{genlem1}, since $\a \in G\cup N$.  
Thus, for $\beta \in
[0, \bar\beta]$
\[
...
\bar{\beta}\right]}$ satisfies the properties of the lemma.
\end{proof}
\newpage
...
The set $N$ is an open subset of $(0, \infty)$ by continuous dependence on the
initial data as given by \#. For $\hat{\a}$ close to $\a$, we have $u(\hat{r},
\hat{\a})=0$ for some $\hat{r}$ near $r$.

By Lemma \ref{genlem2}, we have $u'(r) < 0$ for all $r > 0$ if $\a \in N$.
Hence, the graph of a solution $u(r, \a)$ with $\a \in N$ cannot be tangent to
the $r$-axis and so the function $\za : N \to (0, \infty)$ is continuous.
