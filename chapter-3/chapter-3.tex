...
\a)=0$. Then % by the {\red IVP} \eqref{c3:ivp} we have 
\be \label{ivpdev0} 
...
Furthermore, if $\a\in N$, ten 
\[ \nu(\za) = \za u'(\za) < 0 \]
by Lemma \# and so $\nu$ has a unique zero $\rho_0$ in $[0,\za]$.

Let $\tau \in (0,\za)$ be the first zero of $w$, which exists by Lemma \#. Since
$w$ satisfies

$w$ ivp with initial data $w(0) = 1$ and $\underset{r\to 0}{\lim} rw'(r) = 0$

by Sturm comparison theorem, see Theorem \#, we know that $\nu$ oscillates
faster than $w$. Then $\rho_0 \in (0,\tau)$ and thus
...
However, if $\za = \infty$, we can apply Lemma 6 of \cite[kwong]. The
...
\infty)$ and Lemma 6 of \cite[kwong] implies that
\[ \underset{r\to\infty}{\lim} w(r) = -\infty. \]
